...
# Define data types
type_parent_dict=Protein.class_type.instance_type
type_parent_idx=typeof(typed.List.empty_list(types.int64))
protein_resource_specs=typed.Dict.empty(key_type=types.string,value_type=type_parent_dict)
protein_index_specs=typed.Dict.empty(key_type=types.string,value_type=type_parent_idx)
specs=[
    ('pep_seq',types.string),
    ('_peptide',types.string),
    ('_parent_proteins',typeof(protein_resource_specs)),
    ('_mapped_in_parent_proteins',typeof(protein_index_specs))
]
# define the class 
@jitclass(spec=specs)
class Peptide: 
	...
	def __init__(self,pep_seq: str)->Peptide:
		...
		self._mapped_in_parent_proteins=typed.Dict.empty(key_type=types.string,value_type=type_parent_dict)
		self._parent_proteins=typed.Dict.empty(key_type=types.string,value_type=type_parent_idx)
	...
	def add_parent_protein(self, parent_protein:Protein, 
		...
		:type start_index: int 
		"""
		self._parent_proteins[parent_protein.get_id()]=parent_protein
		index=typed.List.empty_list(types.int64)
		index.append(start_index);index.append(end_index)		
		self._mapped_in_parent_proteins[parent_protein.get_id()]=index
	...
	def get_flanked_peptide(self,flank_len:int)-> Sequences: 
		...
		for prot_id in self._parent_proteins.keys():
			protein_seq=self._parent_proteins[prot_id]
			parent_indx=self._mapped_in_parent_proteins[prot_id]
			# check that the flaking region + protein length is within the protein length.
			if parent_indx[0] - flank_len < 0: 
				start_idx=0
			else: 
				start_idx=parent_indx[0] - flank_len
			# check the C-terminal side 
			if flank_len+parent_indx[1] > len(protein_seq._seq): 
				end_idx=len(protein_seq._seq)
			else: 
				end_idx= flank_len+parent_indx[1]	
	...
	def map_to_parent_protein(self) ->MappedProtein:
		...
		for prot in self._parent_proteins.keys(): 
			...
			pep_index=self._mapped_in_parent_proteins[prot]
			temp_array=np.zeros(shape=(1,len(pp._seq)))
			temp_array[0,pep_index[0]:pep_index[1]]= 1
	...
	def get_non_presented_peptides(self, length: int )->Sequences: 
		...
		for pro in self._parent_proteins.keys():
			results.append(self._parent_proteins[pro].get_non_presented_peptide(self._mapped_in_parent_proteins[pro][0],self._mapped_in_parent_proteins[pro][1], length=length))
		return results
	
	def get_parent_proteins(self)->List[str]:
 		""" return a list of protein unique parent

 		Returns:
            List[str]: [description]
        """
 		return list(self._parent_proteins.keys()) 
	...
	def get_pos_in_parent(self,pro_id:str)->Range:
		...
		if not self.is_child_of(pro_id):
			raise ValueError("The provided protein id is not a parent of the current peptide")
		# get the start and end elements 
		start=self._mapped_in_parent_proteins[pro_id][0]
		end=self._mapped_in_parent_proteins[pro_id][1]
		# return the results 
		return start, end 
	
	def get_parent(self,pro_id:str)->Protein:
		"""
		...
		if self.is_child_of(pro_id):
			return self._parent_proteins[pro_id]
	...
	def get_n_terminal_flank_seq(self, flank_len : int)->List[str]:
		...
		for parent in self._parent_proteins.keys(): 
			...
			start_index=self._mapped_in_parent_proteins[parent][0]
			if start_index-flank_len<0: 
				results.append(parent_protein[0:start_index])
			else: 
				results.append(parent_protein[start_index-flank_len:start_index])
		# return the results 
		return results 
	...
	def get_c_terminal_flank_seq(self, flank_len: int)->List[str]:
		...
		for parent in self._parent_proteins.keys(): 
			...
			end_index=self._mapped_in_parent_proteins[parent][1]
			# get the protein length  
			protein_length=len(parent_protein._seq) 		
			if end_index+flank_len>protein_length: # ==> avoid indexing outside of the parent protein
				results.append(parent_protein[end_index:protein_length])
			else: 
				results.append(parent_protein[end_index:end_index+flank_len])
		# return the results 
		return results 
	...
	def add_org_2_parent(self,prot_name: str,org:str)->None:
		...
		if not self.is_child_of(prot_name):
			raise ValueError('The provided protein id is not a parent of the current peptide')
		self._parent_proteins[prot_name].set_org(org)
	...
	def get_parents_org(self)->Organisms:
		...
		for protein in self._parent_proteins.keys():
			results.append(self._parent_proteins[protein].get_org())
		return results 
