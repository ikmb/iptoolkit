...
public class Tomtom extends SubmitJob<Tomtom.Data> {
  ...
  protected void submitOpalJob(UUID uuid, Data data, HttpServletResponse response)
	...
	FileLock fileLock = null;
        ...
        // set the Tomtom semaphore file name
        String catalinaHome = System.getProperty("catalina.home");
        String outputPrefix = catalinaHome + File.separator + "webapps" + File.separator;
        String semaphoreFileName = outputPrefix + "/opal-jobs/tomtom_lock_file";

        // sleep until the semaphore file can be locked exclusively
        try {
          int nprocs = 3;		// Number of concurrent tomtoms
          int max_tries = 20;		// Number of tries before giving up.
          int duration = 5;		// Time to wait in seconds before trying.
          int i;
          // Loop until you get a lock or run out of tries.
          for (i=0; i<max_tries && (fileLock = lockFile(semaphoreFileName, nprocs)) == null; i++) {
            Thread.sleep(duration * 1000);
          }
          if (fileLock==null) {
            throw new ServletException("Too many Tomtom jobs are running currently.  " + 
             "Please try Tomtom again later or un-check the 'Run immediately' box.");
          } else {
            // Start Tomtom.
            tomtom = pb.start();
          }
        ...
        try {
          waiter.join();
          stillAlive = waiter.isAlive();
          try { 
            // Only assign an exit value if the process has stopped running
            if (!stillAlive) {
              exitValue = tomtom.exitValue(); 
            }
          ...
          // Destroy doesn't work on Ubuntu
          tomtom.destroy();
          stillAlive = waiter.isAlive();
          // Only close the semaphore file if it is open and the process has completed.
          // This will prevent us from getting more than three Tomtom jobs in this state.
          if (fileLock != null && !stillAlive) { 
            fileLock.release(); 
        ...
        if (exitValue == null) {
          ...
          // TLB; Useful error messages will be in the "Error Messages" file 
          // so don't throw an exception!
          // throw new ServletException("Tomtom returned non-zero exit status.");
        }
        // check that a index.html output was created
